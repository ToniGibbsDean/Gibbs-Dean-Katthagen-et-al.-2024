...
# load data
    pfiles<-list.files(file.path("Data", "GenPop2", "Parameters"))
    tfiles<-list.files(file.path("Data", "GenPop2", "TaskData"))
    dfiles<-list.files(file.path("Data", "GenPop2", "Symptom_DemoData"))
    afiles<-list.files(file.path("Data", "GenPop2", "AttentionChecks"))

    p<-lapply(file.path("Data", "GenPop2", "Parameters", pfiles), read.csv, header=TRUE)
    names(p)<-substr(pfiles, start=1, stop=nchar(pfiles)-4)

    t<-lapply(file.path("Data", "GenPop2", "TaskData", tfiles), read.csv)
    names(t)<-tfiles

    d<-lapply(file.path("Data", "GenPop2", "Symptom_DemoData", dfiles), read.csv)
    names(d)<-dfiles

    a<-lapply(file.path("Data", "GenPop2", "AttentionChecks", afiles), read.csv)
    names(a)<-afiles
...
#combine data
    ...
    #append demos
        ...
        d_combined<-full_join(d[[1]], d[[2]], by=c("Participant.Private.ID")) %>%
        ...
        #exclude based on trial performance and symptom scale attention
            #get table of all reasons for exclusion for each participant
                ExclusionBasis<- tibble %>%
                    ...
                    summarise(  nomoveX=sum(distanceMovedX==0), 
                                ...
                                #attentionCheck1= sum(MASQ_1=="NO" & MASQ_2=="NO" & GREEN=="NO")) %>%
            ...
            #extract indices of participants to exclude
                IndicesToExclude<-ExclusionBasis %>%
                                dplyr::select(starts_with("exclude")) %>%
                                rowSums()>0
...
tibble<-    tibble %>%
            mutate(nextPP = lead(participantPosition)) %>%
            mutate(signedPositionChange = nextPP - participantPosition) %>%
            mutate(positionChange = abs(nextPP - participantPosition)) %>%
            #mutate(signedPE = originalResult - participantPosition) %>%
            mutate(LR = signedPositionChange / signedPE)

saveRDS(tibble, file = "Outputs/GenPop2Tibble_new.rds")
