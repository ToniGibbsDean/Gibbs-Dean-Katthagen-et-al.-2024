...
MuMIn::model.sel(mostsimple, LR_fullmod, LR_reduced_mod)
MuMIn::r.squaredGLMM(mostsimple)
MuMIn::r.squaredGLMM(LR_fullmod)
MuMIn::r.squaredGLMM(LR_reduced_mod)
  

   plot1<-   df_mod %>%
                group_by(Participant.Private.ID, level) %>%
                        ggplot(aes(x=level, y=beta, fill=level)) +
                        geom_boxplot() +
                theme_classic()

ggsave(plot1, file=file.path(path, "LRsbyLevel_SPQ.pdf"))
